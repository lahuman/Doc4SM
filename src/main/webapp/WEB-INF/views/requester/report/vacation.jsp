...
<script type="text/javascript">
	...
	function report(){
	    ...
	    if (row && $('#dg').datagrid('getSelected').ID != undefined){
			location.href = "/files/vacationReport.do?ID="+row.ID;
	    }else{
			$.messager.alert('INFO',"출력할 항목을 선택 하세요.",'info');
		}
	} 	 
	function reportList(){
		location.href = "/files/vacationList.do?param="+getForm2Json($('#ff').serializeArray());
	}
	...
	function success(data, textStatus, jqXHR ){
	 ...
	 var vacationKind = {};
	 $(function(){
			$('#VACATION_USER').combobox('reload', '/requester/user/userList.json');
			callAjax2Json("/requester/code/codeList.json?CODE_MASTER=VACA", "", function(vacationData){
			 ...
			 callAjax2Json("/requester/user/userList.json", "", function(data){
				...
				$("#dg").datagrid({
					...
					, singleSelect: true
					, toolbar: '#tb'
					, url: '/requester/vacation/list.json'
...
<body>
	 <!-- Page title -->
      <div class="page-title">
         <div class="container">
            <h2><i class="fa fa-desktop color"></i> VACATION <small>휴가 출력</small></h2>
            <hr />
         </div>
      </div>
      <!-- Page title -->
			...
			<div class="row">
				...
				<div id="tb" style="height:auto;">
					<form id="ff" method="post">
						...
						<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="reportList()">대장저장</a>-
						                &nbsp;&nbsp;
