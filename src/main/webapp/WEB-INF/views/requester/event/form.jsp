...
<script type="text/javascript">
	 ...
	 function goProcessForm(type){
			$("#ff").attr("action", "/requester/event/addEventForm.do");
	 ...
	 function changeForm(type, data){
		$("#TYPE").val(type);
		$("#formArea").load("/requester/event/pop/"+type+"Form.do",{"param":getForm2Json($('#ff').serializeArray())}, function(){
			setTimeout(function(){
				$('#SERVICE_ID').combobox('reload', '/requester/service/serviceList.json?USE_YN=Y');
				if($("#TYPE").val() == "event"){
					$('#EMERGENCY_CODE').combobox('reload', '/requester/code/codeList.json?CODE_MASTER=EMGC');
				}else{
					$('#USER_ID').combobox('reload', '/requester/requester/requesterList.json');
				}
				if(data){
					$('#ff').form('load', data);
				}else{
				...
				if($('#SERVICE_ID').combobox('getValue') != "")
					$('#CATEGORY_ID').combobox('reload', '/requester/service/categoryList.json?SERVICE_ID='+$('#SERVICE_ID').combobox('getValue'));
				$("#USER").val("<%=request.getAttribute("name")%>");
				$("#USER_TEL").val("<%=request.getAttribute("tel")%>");
			}, 1000);
		});
	 ...
	 function submitForm(){
		 if($('#ff').form('validate')){
			 ...
			 //submit
			 callAjax2Json("/requester/event/addEvent.do",{"param":getForm2Json($('#ff').serializeArray())}, success );
		 }
	 }
	 ...
	 function deleteFile(type){
		...
		function downloadFile(type){
			...
			location.href="/requester/event/attattachFile.do?REQ_CODE="+$("#REQ_CODE").val()+"&TYPE="+type;
		}
		var fileType = "";
		function uploadFile(type){
			fileType = type;
			popupWindow = window.open(
	    			"/requester/event/pop/doFileUpload.do?REQ_CODE="+$("#REQ_CODE").val()+"&TYPE="+type,'uploadFilePop','height=200,width=300,left=300,top=100,resizable=no,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=no');
		}
