...
<script type="text/javascript">
	            ...
	            if (row){  
        			...
        			if(rows.length != 0 && validateGrid(rows)){
		                $.messager.confirm('Confirm','변경하신 내용을 저장 하시겠습니까?',function(r){  
		                    if (r){  
	            				callAjax2Json("/admin/problem/reationProcess.do",makeParam(rows), success );
		                    }  
		                });  
           			}
	            }
	...
	function success(data, textStatus, jqXHR ){
	 ...
	 $(function(){
		 callAjax2Json("/admin/code/codeList.json?CODE_MASTER=PBLM", "", function(conditionData){
			 conditionList = conditionData;
			 callAjax2Json("/admin/problem/eventList.json?YEAR="+(new Date).getFullYear() , "", function(eventData){
				 eventList = eventData;
				$("#dg").datagrid({
					title:"문제 연관 이벤트 관리"
						, width:780
						, height:500
						, url:"/admin/problem/reationList.json?PROBLEM_ID=<%=request.getParameter("PROBLEM_ID")%>"
