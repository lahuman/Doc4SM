...
var isProcess = false;
	$(function(){
		 ...
		 <%}else if(!"".equals(Utils.getRequestValue(request.getParameter("REQ_CODE")))){%>
		 	callAjax2Json("/admin/event/read.json", {"param":JSON.stringify({REQ_CODE:"<%=request.getParameter("REQ_CODE")%>"})}, function(data){
	...
	function goProcessForm(type){
		$("#ff").attr("action", "/admin/event/pop/processEvent.do");
		$("#param").val(makeForm2Json());
		$("#ff").submit();
	}
	
	function changeForm(type, modifyData){
		$("#formArea").load("/admin/event/pop/"+type+"Form.do",{"param":makeForm2Json()}, function(){
			setTimeout(function(){
				...
				$('#INFRA_ID').combobox('reload', '/admin/infra/comboInfraList.json');
	...
	function submitForm(){
		 if($('#ff').form('validate')){
			...
			//submit
			callAjax2Json("/admin/event/processEvent.do",{"param":getForm2Json($('#ff').serializeArray())}, success );
