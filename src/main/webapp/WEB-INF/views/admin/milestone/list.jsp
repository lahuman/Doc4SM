	            ...
	            if (row){  
        			...
        			if(rows.length != 0 && validateGrid(rows)){
		                $.messager.confirm('Confirm','변경하신 내용을 저장 하시겠습니까?',function(r){  
		                    if (r){  
	            				callAjax2Json("/admin/milestone/process.do",makeParam(rows), success );
		                    }  
		                });  
           			}
	            }
		 ...
		 if (endEditing()){
            ...
            if (row && $('#dg').datagrid('getSelected').ID != undefined){
            		popupWindow = window.open(
            			"/admin/pop/milestoneRelation.do?MILESTONE_ID="+row.ID,'popUpWindow','height=510,width=810,left=10,top=10,resizable=no,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=no')
            }else{
