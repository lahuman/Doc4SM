...
<script type="text/javascript">
	 ...
	 $(function(){
				$("#dg").datagrid({
					title:"연관 이벤트 관리"
						, width:780
						, height:500
						, url:"/admin/milestone/reationList.json?MILESTONE_ID=<%=request.getParameter("MILESTONE_ID")%>"
						, iconCls: 'icon-edit'
						, toolbar: '#tb'
						, singleSelect: true
						, columns:[[
	 ...
	 function formatDetail(value,row){
		 	...
		 	if(row.EVENT_TYPE == "업무"){
		 		href = '/admin/event/pop/processEvent.do?REQ_CODE='+row.EVENT_CODE;
		 	}else if(row.EVENT_TYPE == "단순"){
		 		href = '/admin/call.do?ID='+row.EVENT_CODE;
		 	}else if(row.EVENT_TYPE == "외부"){
		 		href = '/admin/external.do?ID='+row.EVENT_CODE;
		 	}
