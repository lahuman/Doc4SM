...
function showDetail(){
    ...
    if (row && $('#dg').datagrid('getSelected').SVN_VERSION != undefined){
    		popupWindow = window.open(
    			"/user/version/pop/showDetail.do?REVISON="+row.SVN_VERSION,'popUpWindow','height=450,width=600,left=10,top=10,resizable=no,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=no')
    }else{
	            ...
	            if (row){  
        			...
        			if(rows.length != 0 && validateGrid(rows)){
		                $.messager.confirm('Confirm','변경하신 내용을 저장 하시겠습니까?',function(r){  
		                    if (r){  
	            				callAjax2Json("/user/version/process.do",makeParam(rows), success );
		                    }  
		                });  
           			}
	            }
	...
	function success(data, textStatus, jqXHR ){
	 ...
	 $(function(){
			$("#dg").datagrid({
				...
				, singleSelect: true
				, toolbar: '#tb'
				, url: '/user/version/manage.json'
				, onDblClickCell: onClickRow
				, columns:[[
					{field:'KIND_CD', title:"작업구분", width: 80, align:'center', formatter:function(value,row){  
