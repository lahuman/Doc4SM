...
<script type="text/javascript">
	 ...
	 function goProcessForm(type){
			$("#ff").attr("action", "/user/event/addEventForm.do");
	 ...
	 function changeForm(type, data){
		$("#TYPE").val(type);
		$("#formArea").load("/user/event/pop/"+type+"Form.do",{"param":getForm2Json($('#ff').serializeArray())}, function(){
			setTimeout(function(){
				...
				if($("#TYPE").val() == "event"){
					...
					$('#USER_ID').combobox('reload', '/user/user/userList.json');
	 ...
	 function submitForm(){
		 if($('#ff').form('validate')){
			 ...
			 //submit
			 callAjax2Json("/user/event/addEvent.do",{"param":getForm2Json($('#ff').serializeArray())}, success );
		 }
	 }
