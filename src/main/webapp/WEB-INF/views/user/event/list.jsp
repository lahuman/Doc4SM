...
<script type="text/javascript">
	 ...
	 function connectPopup(){
	        ...
	        if (row && $('#dg').datagrid('getSelected').REQ_CODE != undefined){
	        		popupWindow = window.open(
	        			"/user/event/pop/processEvent.do?REQ_CODE="+row.REQ_CODE,'popUpWindow','height=500,width=660,left=10,top=10,resizable=no,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=no')
	        }else{
	 ...
	 function commentPopup(){
        ...
        if (row && $('#dg').datagrid('getSelected').REQ_CODE != undefined){
        		popupWindow = window.open(
        			"/user/event/pop/commentList.do?REQ_CODE="+row.REQ_CODE,'popUpWindow','height=500,width=660,left=10,top=10,resizable=no,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=no')
        }else{
		            ...
		            if (row){  
	        			...
	        			if(rows.length != 0 ){
			                $.messager.confirm('Confirm','변경하신 내용을 저장 하시겠습니까?',function(r){  
			                    if (r){  
		            				callAjax2Json("/user/event/changeStatus.do",makeParam(rows), success );
			                    }  
			                });  
	           			}
		            }
